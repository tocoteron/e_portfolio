		...
		else
		{
			...
			foreach($evaluation as $category_id => $category)
			{
				foreach($category as $item_id => $item)
				{
					...
					$stmt->bindValue(":class_id", $class_id);
					$stmt->bindValue(":evaluator_id", $user_id);
			...
			$stmt = $dbh->prepare($sql);
			$stmt->bindValue(":class_id", $class_id);
			$stmt->bindValue(":commenter_id", $user_id);
			$stmt->bindValue(":user_id", $user_id);
			$stmt->bindValue(":assignment_id", $assignment_id);
...
// 提出済み課題のIDリストを取得
$sql_for_get_submitted_assginment_ids = "SELECT assignment_id FROM evaluations WHERE evaluator_id = :user_id GROUP BY assignment_id";
